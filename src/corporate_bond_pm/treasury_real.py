...
def fetch_treasury_yield_curve_year(year: int) -> pd.DataFrame:
    req = Request(
        TREASURY_XML.format(year=int(year)),
...
def fetch_treasury_yield_curve_range(start: date | str, end: date | str) -> pd.DataFrame:
    ...
    frames = [
        fetch_treasury_yield_curve_year(y)
...
def _numeric_tenor(column) -> float | None:
...
def interpolate_curve_row(row: pd.Series, target_years: float) -> float:
    pts = []
    for c in row.index:
        tenor = _numeric_tenor(c)
        if tenor is None or pd.isna(row[c]):
            continue
        try:
            pts.append((tenor, float(row[c])))
        except (TypeError, ValueError):
            continue
    pts.sort()
    if len(pts) < 2:
...
def attach_treasury_proxy(
    ...
    *,
    ...
    merged = pd.merge_asof(
        x, t, on="date", direction="backward",
        tolerance=pd.Timedelta(days=tolerance_days),
    )
    call_ts = pd.Timestamp(call_date).normalize()
    horizon_date = (
        pd.to_datetime(merged["settlement_date"]).dt.normalize()
        if "settlement_date" in merged.columns
        else merged["date"]
    )
    merged["years_to_call"] = (call_ts - horizon_date).dt.days / 365.25
    merged["treasury_yield_pct"] = merged.apply(
        lambda r: interpolate_curve_row(r, r["years_to_call"])
        if r["years_to_call"] > 0 else np.nan,
        axis=1,
    )
    return merged
