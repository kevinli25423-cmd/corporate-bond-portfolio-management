...
DATE_CANDIDATES = {"tradedate", "executiondate", "date", "effectivedate", "transactiondate", "tradeexecutiondate"}
PRICE_CANDIDATES = {"price", "tradeprice", "executionprice", "lastprice", "reportedprice"}
VOLUME_CANDIDATES = {"quantity", "tradequantity", "volume", "tradevolume", "parvalue", "principalamount", "size"}
...
def _parse_volume(value) -> float:
    ...
    try:
        ...
        return float(m.group()) * multiplier if m else np.nan


def load_finra_trade_export(path: str | Path, *, date_col: str | None = None, price_col: str | None = None, volume_col: str | None = None) -> pd.DataFrame:
    """Normalize a user-exported FINRA Fixed Income trade-history CSV."""
    path = Path(path)
    df = pd.read_csv(path)
    if df.empty:
    ...
    out["price"] = pd.to_numeric(df[price_col].astype(str).str.replace(r"[^0-9.\-]", "", regex=True), errors="coerce")
    out["displayed_volume"] = df[volume_col].map(_parse_volume) if volume_col is not None else np.nan
...
def daily_trade_summary(trades: pd.DataFrame, *, representative: str = "median") -> pd.DataFrame:
    if representative not in {"median", "vwap"}:
        raise ValueError("representative must be 'median' or 'vwap'")
    ...
    for d, g in trades.groupby("date"):
        ...
        vwap = np.nan
        if valid_vol.any():
            vwap = float(np.average(g.loc[valid_vol, "price"], weights=g.loc[valid_vol, "displayed_volume"]))
        med = float(g["price"].median())
        rep = med if representative == "median" or np.isnan(vwap) else vwap
        rows.append({
            "date": d,
            "trade_count": len(g),
